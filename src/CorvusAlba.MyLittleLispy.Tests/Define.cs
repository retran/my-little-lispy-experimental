...
namespace CorvusAlba.MyLittleLispy.Tests
{
    public class Define
    {
        ...
        {
            Assert.Equal(Cons.Empty, _engine.Evaluate("(define ten 10)"));
            Assert.Equal(Cons.Empty, _engine.Evaluate("(define twenty 20)"));
            Assert.Equal(Cons.Empty, _engine.Evaluate("(define thirty 30)"));

            Assert.Equal(Cons.Empty, _engine.Evaluate("(define expr (+ ten 15))"));
        ...
        public void RedefinedGlobalShouldEvaluateToItsNewValue()
        {
            ...
            Assert.Equal(Cons.Empty, _engine.Evaluate("(define ten 100)"));
